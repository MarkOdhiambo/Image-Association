...
fore = Image.open('0010.jpg')
base_width = 360
width_percent = (base_width / float(fore.size[0]))
hsize = int((float(fore.size[1]) * float(width_percent)))
fore = fore.resize((base_width, hsize), PIL.Image.ANTIALIAS)
fore = asarray(fore)
...
"Using the mean shift based output as mask"
# images=io.imread('0010.jpg')
# fore = color.rgb2gray(fore)
# val = filters.threshold_otsu(image)
model_image = color.rgb2gray(model_image)
val = filters.threshold_otsu(model_image)
mask = model_image > val
model_image[mask]=1
...
plt.clf()
plt.axis('off')
plt.title('Mask Image', loc='center')
...
plt.clf()
plt.axis('off')
plt.title('Segmented Image', loc='center')
plt.imshow()
